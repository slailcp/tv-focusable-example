<template>
  <div class="tv-box">
    ...
    <div class="loading" v-show="loadingShow"><img src="../assets/tv/loadingCancel.gif"/></div>
  </div>
</template>
  ...
  created() {
    ...
    this.$nextTick(() => {
      ...
      this.$tv.setScrollEl(document.querySelector('.item-box'))
        this.$tv.requestFocus(this.$tv.getElementByPath("//div[@class='perspective']/div[3]"));
    })
  },
  destroyed() {
...
.item{
  ...
  img{width:350px;height:569px;border-radius:30px;box-shadow:1px 1px 43px rgba(198, 184, 255, 0.8);border:3px solid #4a75a2;box-sizing: border-box;}
  .txt{text-align: center;display: inline-block;width:100%;line-height:46px;color:#fff;}
